    ...
    : AudioProcessorEditor(processor), processor(processor)
{
    ...
    openFile->setImages (false, true, true,
                         ImageCache::getFromMemory (folder_png, folder_pngSize), 1.000f, Colour (0x00000000),
                         Image(), 1.000f, Colour (0x00000000),
                         Image(), 1.000f, Colour (0x00000000));
    ...
    addAndMakeVisible (repeat = new ToggleButton ("repeat"));
...
void AudioPlayerEditor::resized()
{
    ...
    back->setBounds (64, 8, 40, 24);
    playPause->setBounds (112, 8, 40, 24);
    stop->setBounds (152, 8, 40, 24);
    openFile->setBounds (8, 8, 40, 24);
    timeSlider->setBounds (8, 40, 184, 24);
    repeat->setBounds (200, 8, 71, 24);
    timeLabel->setBounds (200, 40, 72, 24);
...
void AudioPlayerEditor::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    if (buttonThatWasClicked == back)
    {
		...
		processor.getAudioSource()->setNextReadPosition(0);
    ...
    else if (buttonThatWasClicked == playPause)
    {
		...
		else if (getTransportState() == TransportState::Playing)
    ...
    else if (buttonThatWasClicked == openFile)
    {
		...
		{
			Logger::outputDebugString("open file");
            ...
            if (!processor.openFile(chooser.getResult())) {
...
void AudioPlayerEditor::sliderValueChanged (Slider* sliderThatWasMoved)
{
    ...
    if (sliderThatWasMoved == timeSlider)
    {
		...
		int64 pos = int(timeSlider->getValue() / 100 * processor.getAudioSource()->getTotalLength());
		processor.getAudioSource()->setNextReadPosition(pos);
		updateTime(); // to update slider and label
		if (pos > 0)
...
void AudioPlayerEditor::togglePlayPause(bool play)
{
	playPause->setImages(false, true, true,
...
{
	if (processor.getAudioSource() != nullptr)
		processor.getAudioSource()->setLooping(shouldLoop);
}

void AudioPlayerEditor::updateTime() {
	int64 pos = processor.getAudioSource()->getNextReadPosition();

	if (pos != previousPos) {
		previousPos = pos;

		int64 len = processor.getAudioSource()->getTotalLength();

        // IMPORTANT! don't send notification, as this would trigger sliderValueChanged -> setNextReadPosition (which causes kinda notification feedback loop)
        timeSlider->setValue((double)pos / len * 100, dontSendNotification);

        // calculate and display time
        const RelativeTime rPos((double)pos/44100);
...
void AudioPlayerEditor::timerCallback() {
	...
	static TransportState oldState = TransportState::Stopped;
...
<JUCER_COMPONENT documentType="Component" className="AudioPlayerEditor" componentName=""
                 parentClasses="public AudioProcessorEditor, private ChangeListener, private Timer, public ButtonListener, public SliderListener"
                 ...
                 fixedSize="1" initialWidth="285" initialHeight="75">
  <BACKGROUND backgroundColour="ffffffff"/>
  <IMAGEBUTTON name="back" id="6004726561a75f83" memberName="back" virtualName=""
               explicitFocusOrder="1" pos="64 8 40 24" buttonText="" connectedEdges="0"
  ...
  <IMAGEBUTTON name="playPause" id="30084e5ef045db45" memberName="playPause"
               virtualName="" explicitFocusOrder="2" pos="112 8 40 24" buttonText=""
  ...
  <IMAGEBUTTON name="stop" id="254c53ebeb5bf50b" memberName="stop" virtualName=""
               explicitFocusOrder="3" pos="152 8 40 24" buttonText="" connectedEdges="0"
               needsCallback="1" radioGroupId="0" keepProportions="1" resourceNormal="stop_png"
               opacityNormal="1" colourNormal="0" resourceOver="" opacityOver="1"
               colourOver="0" resourceDown="" opacityDown="1" colourDown="0"/>
  <IMAGEBUTTON name="openFile" id="ef85c0e317ed4a3b" memberName="openFile" virtualName=""
               explicitFocusOrder="0" pos="8 8 40 24" buttonText="" connectedEdges="0"
               needsCallback="1" radioGroupId="0" keepProportions="1" resourceNormal="folder_png"
               opacityNormal="1" colourNormal="0" resourceOver="" opacityOver="1"
               colourOver="0" resourceDown="" opacityDown="1" colourDown="0"/>
  <SLIDER name="timeSlider" id="9ea57500d69f6164" memberName="timeSlider"
          virtualName="" explicitFocusOrder="4" pos="8 40 184 24" thumbcol="ffa6a6b4"
  ...
  <TOGGLEBUTTON name="repeat" id="91f83c69cc9f462a" memberName="repeat" virtualName=""
                explicitFocusOrder="0" pos="200 8 71 24" buttonText="repeat"
                connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
  <LABEL name="timeLabel" id="906dcb6c6fd6022" memberName="timeLabel"
         virtualName="" explicitFocusOrder="0" pos="200 40 72 24" edTextCol="ff000000"
